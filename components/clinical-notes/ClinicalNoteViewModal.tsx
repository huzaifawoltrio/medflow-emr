...
const mockAmendments: Amendment[] = [
  {
    ...
    note_id: 1,
    amendment_text:
      "Updated medication dosage after patient reported side effects.",
...
export const ClinicalNoteViewModal: React.FC<ClinicalNoteViewModalProps> = ({
  ...
  const [selectedNote, setSelectedNote] = useState<ClinicalNote | null>(note);
  ...
  // Simulate loading note details and amendments when modal opens
  useEffect(() => {
    if (isOpen && note?.id) {
  ...
  useEffect(() => {
    if (signSuccess) {
      // Simulate reloading note details
      setNotesLoading(true);
      setTimeout(() => {
        if (selectedNote) {
          setSelectedNote({

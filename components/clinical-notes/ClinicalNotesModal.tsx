...
export const ClinicalNotesModal: React.FC<ClinicalNotesModalProps> = ({
  ...
  }, [isOpen, templates.length, dispatch]);
  ...
  const handleSubmit = async (action: "draft" | "sign") => {
    ...
    try {
      const result = await dispatch(createClinicalNote(noteData));

      if (createClinicalNote.fulfilled.match(result) && action === "sign") {
        // If we want to sign the note immediately after creation, we would need to call signClinicalNote
        // For now, we'll create as draft and the user can sign it later
        // You can implement auto-signing here if needed
      }
    } catch (error) {
